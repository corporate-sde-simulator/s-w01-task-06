...
> **Welcome to Service-Track Week 1, Task 6!**
...
| `src/attendanceReport.js` | Main source code - **has bugs to fix** |
| `src/reportFormatter.js` | Supporting code - **may also have bugs** |
| `tests/attendanceReport.test.js` | Test file - **add your tests here** |
...
### Bug #1 - in `src/attendanceReport.js` (around line 66)
**What's wrong:** Doesn't handle overnight shifts

**How to find it:** Open `src/attendanceReport.js` and look for the `BUG:` comment near line 66. Read the comment carefully - it describes exactly what's broken.
...
### Bug #2 - in `src/attendanceReport.js` (around line 114)
**What's wrong:** Attendance percentage uses 7 days as denominator

**How to find it:** Open `src/attendanceReport.js` and look for the `BUG:` comment near line 114. Read the comment carefully - it describes exactly what's broken.
